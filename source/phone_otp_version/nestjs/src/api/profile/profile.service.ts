...
import {emptyListRes, isValidEmail} from "../../core/utils/utils";
import {isNumericString} from "../../core/utils/app.validator";
...
export class ProfileService {
    ...
    async getMyProfile(user: IUser,authId:string) {
        let res = {};
        res["me"] = await this.userService.findById(user._id, "-lastMail  ");
        res["me"]['authMethod'] =res["me"]
            .authMethods
            .find((method) => method.identifier == authId.toString());
        delete res["me"]['authMethods']
    ...
    async updatePassword(user: IUser, dto: UpdatePasswordDto) {
        let foundedUser: IUser = await this.userService.findByIdOrThrow(dto.myUser._id, "authMethods");
        let emailMethod: any = foundedUser.authMethods.find((method) => method.type === 'email' && method.identifier === foundedUser.email);
        let bcryptRes = await bcrypt.compare(dto.oldPassword, emailMethod.password);
        if (!bcryptRes) {
    ...
    async getUsersAndSearch(dto: Object, myUser: IUser) {
        let myPhoneUser = await this.userService.findById(myUser._id, "phone")
        let searchKey = dto['fullName']
        let isNumber = isNumericString(searchKey ?? "");
        if (myPhoneUser.phone) {

            let myUsers = await this.contactsService.findAll({
                userId: myUser._id
            }, "contactPhone")
            let nameOrEmail = dto['fullName'];
            let match: any = {
                phone: {$in: myUsers.map(value => value.contactPhone)},
            }
            ///this means you search by email you've entered a valid email
            if (nameOrEmail && isValidEmail(nameOrEmail)) {
                match = {
                    'email': remove(nameOrEmail).toLowerCase()
                }
                delete dto['fullName']
            }
            /// this means your enter a valid number to be search
            if (isNumber) {
                match = {
                    phone: {$regex: isNumber, $options: 'i'}
                }
                delete dto['fullName']
            }

            ///if all not search inside or rerun all your contact phones
            return this.userService.searchV2(
                dto,
                [myUser._id],
                match
            )
        ...
        let match: object = {
            email: remove(searchKey).toLowerCase()
        }
        if (isNumber) {
            match = {
                phone: {$regex: isNumber, $options: 'i'}
        ...
        delete dto['fullName']
        return this.userService.searchV2(dto, bans, match);
    }
    ...
    async updateMyPassword(dto: UpdateMyPasswordDto) {
        let foundedUser: IUser = await this.userService.findById(
            dto.myUser._id,
            "authMethods userDevice lastMail banTo email registerStatus"
        );
        let emailMethod: any = foundedUser.authMethods.find((method) => method.type === 'email' && method.identifier === foundedUser.email);
        await this.authService.comparePassword(dto.oldPassword, emailMethod.password)
        await this.userService.findByIdAndUpdate(dto.myUser._id, {
    ...
    async checkPassword(user: IUser, password: string) {
        ...
        let foundedUser: IUser = await this.userService.findByIdOrThrow(
            user._id,
            "userDevice lastMail banTo email registerStatus deletedAt authMethods"
        );
        let emailMethod: any = foundedUser.authMethods.find((method) => method.type === 'email' && method.identifier === foundedUser.email);
        await this.comparePassword(password, emailMethod.password)
        return true
    }
